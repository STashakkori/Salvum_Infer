...
pub fn is_lz4(buf: &[u8]) -> bool {
    buf.len() > 3
        && (buf[0] == 0x04 || buf[0] == 0x02)
        && (buf[1] == 0x22 || buf[1] == 0x21)
        && (buf[2] == 0x4D || buf[2] == 0x4C)
        && buf[3] == 0x18
}

/// Returns whether a buffer is a lzma archive.
pub fn is_lzma(buf: &[u8]) -> bool {
    buf.len() > 4
        && buf[0] == 0x5D
        && buf[1] == 0x00
        && buf[2] == 0x00
...
pub fn is_cpio(buf: &[u8]) -> bool {
    ...
    buf.len() > 5
        ...
        && buf[3] == 0x37
        && buf[4] == 0x30
        && (buf[5] == 0x31 || buf[5] == 0x32 || buf[5] == 0x37)
}
...
/// Returns whether a buffer is an lzop archive.
pub fn is_lzop(buf: &[u8]) -> bool {
    buf.len() > 8
        && buf[0] == 0x89
        && buf[1] == 0x4c
        && buf[2] == 0x5a
